...
namespace input {
	...
	void handleKeyRelease(Keys key, std::function<void()> cb) {
		if (isKeyRelease(key)) {
			cb();
		}
	}
	
	void handleKeyPressOnce(Keys key, std::function<void()> cb) {
		if (isKeyPressOnce(key)) {
			cb();
		}
	}
	
	void handleKeyPress(Keys key, std::function<void()> cb) {
		if (isKeyPress(key)) {
			cb();
		}
	}
	
	void handleKeyReleaseOnce(Keys key, std::function<void()> cb) {
		if (isKeyReleaseOnce(key)) {
			cb();
		}
	}
	...
	void handleMouseButtonRelease(MouseButtons mb, std::function<void()> cb) {
		if (isMouseButtonRelease(mb)) {
			cb();
		}
	}

	void handleMouseButtonPressOnce(MouseButtons mb, std::function<void()> cb) {
		if (isMouseButtonPressOnce(mb)) {
			cb();
		}
	}

	void handleMouseButtonPress(MouseButtons mb, std::function<void()> cb) {
		if (isMouseButtonPress(mb)) {
			cb();
		}
	}

	void handleMouseButtonReleaseOnce(MouseButtons mb, std::function<void()> cb) {
		if (isMouseButtonReleaseOnce(mb)) {
			cb();
		}
	}
	...
	void handleMousePosition(std::function<void(MouseCoord&)> cb) {
		cb(mousePosition);
	}

	void handleMouseVelocity(std::function<void(MouseCoord&)> cb) {
		cb(mouseVelocity);
	}
	...
	void handleMouseWheel(std::function<void(MouseWheel& mw)> cb) {
		cb(mouseWheel);
	}
	...
	void handleGrabTrue(std::function<void()> cb) {
		if (isGrab()) {
			cb();
		}
	}
	...
	void handleInputMapRelease(std::string name, std::function<void()> cb) {
		if (isInputMapRelease(name)) {
			cb();
		}
	}
	
	void handleInputMapPressOnce(std::string name, std::function<void()> cb) {
		if (isInputMapPressOnce(name)) {
			cb();
		}
	}
	
	void handleInputMapPress(std::string name, std::function<void()> cb) {
		if (isInputMapPress(name)) {
			cb();
		}
	}
	
	void handleInputMapReleaseOnce(std::string name, std::function<void()> cb) {
		if (isInputMapReleaseOnce(name)) {
			cb();
		}
	}
